...
from
(
select [Sale region], [date], count([site]) as count_site, [Utilization_range] as UR
from dailydata
WHERE [Date] > date('now','-32 day')
group by [Sale region], [date], [Utilization_range]
)
group by [Sale region], [date]